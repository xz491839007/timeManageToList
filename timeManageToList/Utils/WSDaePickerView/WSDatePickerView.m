...
-(instancetype)initWithDate:(WSDateStyle)datePickerStyle StartTime:(NSDate *)startTime endTime:(NSDate *)endTime CompleteBlock:(void(^)(NSDate *))completeBlock{
    self = [super init];
    if (self) {
        ...
        if (endTime) {
            //最大最小限制
            self.maxLimitDate=endTime;
        }
        if (startTime) {
            self.minLimitDate=startTime;
        }
